...
exports.uploadphoto = async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(404)
        .send({ message: "no user found in the data base" });
    ...
    if (req.files) {
      ...
      if (extension === "jpg" || extension === "png" || extension === "jpeg") {
        ...
        let imagepath =
          "http://localhost:5050" + "/userphoto" + imgpath + "/" + filename;
        ...
        fs.writeFileSync(savepath, imagedata, (err) => {
          if (err) {
            console.log(
              "error found while uploading user photo: \n",
              err.message
            );
          }
        });
      ...
      return res
        .status(301)
        .send({ message: "please upload a png/jpeg/jpg file" });
    }
